...
def axiom_generator_percept_sentence(t, tvec):
    ...
    axiom_str = ''
    total = 0
    #use the statements in the example and construct the sentence
    #if not true, add the ~
    for vec in tvec:
        if vec == False:
            if total == 0:
...
def axiom_generator_initial_location_assertions(x, y):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    #use given functions to say wumpus and pit
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    for (i, j) in [((x - 1), y), ((x + 1), y), (x, (y - 1)), (x, (y + 1))]:
        if i >= xmin and i <= xmax and j >= ymin and j <= ymax:
            pit_loc.append(pit_str(i, j))
    pit_loc.append('P' + str(x) + '_' + str(y))
    axiom_str += '{0} <=> ({1})'.format(breeze_str(x, y), (' | ').join(pit_loc))
    return axiom_str
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    for (i, j) in [((x - 1), y), ((x + 1), y), (x, (y - 1)), (x, (y + 1))]:
        if i >= xmin and i <= xmax and j >= ymin and j <= ymax:
            wumpus_loc.append(wumpus_str(i, j))
    wumpus_loc.append('W' + str(x) + '_' + str(y))
    axiom_str += '{0} <=> ({1})'.format(stench_str(x, y), (' | ').join(wumpus_loc))
    return axiom_str
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    #at least one, put them everywhere with an or
    axiom = []
    for x in range(xmin, xmax + 1):
        for y in range(ymin, ymax + 1):
            axiom.append(wumpus_str(x, y))
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    #*** YOUR CODE HERE ***
    axiom = []
    board = [(i, j) for i in range(xmin, xmax + 1) for j in range(ymin, ymax+1)]

    for space in board:
        notHere = [location for location in board if location != space]
        noWumpus = ['~' + wumpus_str( wumpus[0], wumpus[1]) for wumpus in notHere]

        axiom.append('({0} >> ({1}))'.format(wumpus_str(space[1], board[1]), '&'.join(noWumpus)))
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    """
    axiom_str = ''
    #traverse the board, for each time t, use the given function to say we are in that spot
    axioms = []
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    axiom_str = ''
    #"*** YOUR CODE HERE ***
    #if going one direction cannot go any other simple string formating
    if heading == 'north':
...
def axiom_generator_have_arrow_and_wumpus_alive(t = 0):
    ...
    """
    axiom_str = ''
    #use arrow and wumpus alive function
...
def axiom_generator_location_OK(x, y, t):
    ...
    """
    axiom_str = ''
    "no pit at location and no wumpus but can move into there if wumpus is dead"
    #"*** YOUR CODE HERE ***"
    axiom_str = '{0} <=> (~{1} & (~{2} | ({2} & ~{3})))'.format(state_OK_str(x, y, t), pit_str(x, y), wumpus_str(x, y), state_wumpus_alive_str(t))
    return axiom_str
...
def axiom_generator_breeze_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    #perceving a breeze implies breeze
...
def axiom_generator_stench_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    #when a stench is percived it implies there is a stench
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    moves = ['({0} & (~{1} | {2} | {3} | {4} | {5} | {6}))'.format(state_loc_str(x, y, t), action_forward_str(t), action_turn_right_str(t), action_turn_left_str(t), action_grab_str(t), action_shoot_str(t), percept_bump_str(t + 1))]

    for ((i,j),action) in [(((x - 1), y), 'east'), (((x + 1), y), 'west'), ((x, (y - 1)), 'north'), ((x, (y + 1)), 'south')]:
        if i >= xmin and i <= xmax and j >= ymin and j <= ymax:
    ...
    successor = state_loc_str(x, y, t + 1)
    axiom_str = '{0} <=> ({1})'.format(successor,' | '.join(moves))
    return axiom_str
...
def axiom_generator_have_arrow_ssa(t):
    ...
    axiom_str = '' 
    "*** YOUR CODE HERE ***"
    #if we have the arrow at t+1, we have it at t and didnt shoot
...
def axiom_generator_wumpus_alive_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    #if wumpus is alive at t+1 is was alive at t, scream at t+1 if dies
...
def axiom_generator_heading_north_ssa(t):
    ...
    "*** YOUR CODE HERE ***"
    noGo = '({0} & ({1} | {2} | {3} | {4} | {5}))'.format(state_heading_north_str(t), action_wait_str(t), action_grab_str(t), action_shoot_str(t), percept_bump_str(t+1), action_forward_str(t))
    ...
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_north_str(t + 1), noGo, turnLeft, turnRight)
    return axiom_str
...
def axiom_generator_heading_east_ssa(t):
    ...
    "*** YOUR CODE HERE ***"
    noGo = '({0} & ({1} | {2} | {3} | {4} | {5}))'.format(state_heading_east_str(t), action_wait_str(t), action_grab_str(t), action_shoot_str(t), percept_bump_str(t + 1), action_forward_str(t))
    ...
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_east_str(t + 1), noGo, turnLeft, turnRight)
    return axiom_str
...
def axiom_generator_heading_south_ssa(t):
    ...
    """
    axiom_str = ''
    noGo = '({0} & ({1} | {2} | {3} | {4} | {5}))'.format(state_heading_south_str(t), action_wait_str(t), action_grab_str(t), action_shoot_str(t), percept_bump_str(t + 1), action_forward_str(t))
    ...
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_south_str(t + 1), noGo, turnLeft, turnRight)
    "*** YOUR CODE HERE ***"
    return axiom_str
...
def axiom_generator_heading_west_ssa(t):
    ...
    """
    axiom_str = ''
    noGo = '({0} & ({1} | {2} | {3} | {4} | {5}))'.format(state_heading_west_str(t), action_wait_str(t), action_grab_str(t), action_shoot_str(t), percept_bump_str(t + 1), action_forward_str(t))
    ...
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_west_str(t + 1), noGo, turnLeft, turnRight)
    return axiom_str
...
def axiom_generator_heading_only_east(t):
    ...
    "*** YOUR CODE HERE ***"
    #we made a "only heading one direction" function up top, reuse here
...
def axiom_generator_heading_only_west(t):
    ...
    "*** YOUR CODE HERE ***"
    #we made a "only heading one direction" function up top, reuse here
    axiom_str = '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_west_str(t), state_heading_south_str(t), state_heading_east_str(t), state_heading_north_str(t))
    return axiom_str
...
def axiom_generator_only_one_action_axioms(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    action = ['Forward','Grab','Shoot','Climb','TurnLeft','TurnRight','Wait']
    axiom = []

    for i in range(0, len(action)):
        for j in range(i + 1, len(action)):
            axiom.append( '(~{0} | ~{1})'.format(action[i], action[j]))
