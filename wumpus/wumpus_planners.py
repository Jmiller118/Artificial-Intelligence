...
class PlanRouteProblem(search.Problem):
    ...
    def h(self,node):
        ...
        """
        "*** YOUR CODE HERE ***"
        distance = min([manhattan_distance_with_heading(node.state, goal) for goal in self.goals])
        return distance
        #pass
    ...
    def actions(self, state):
        ...
        #state (x,y,heading) 
        #heading 0N, 1W, 2S, 3E
        #list of things do do is forward, left, right
        #can only go up if not on edge
        
        vertical = ["Forward", "TurnRight", "TurnLeft"]
        horizontal = ["TurnRight", "TurnLeft"]
    ...
    def result(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        #whats the new state after forward, rigt, left
        #state - x,y,heading
        #heading 0N, 1W, 2S, 3E
        ...
        if action == 'TurnLeft':
            #facing north, turn to west
            if state[2] == 0:
                return (state[0], state[1], 1)
            #facing south, turn to east
            if state[2] == 2:
                return (state[0], state[1], 3)
            #facing east, turn to north
            if state[2] == 3:
                return (state[0], state[1], 0)
            #facing west, turn to south
            if state[2] == 1:
        ...
        if action == 'TurnRight':
            ...
            if state[2] == 0:
                return (state[0], state[1], 3)
            #facing south,turn  west
            if state[2] == 2:
                return (state[0], state[1], 1)
            #facing east, turn south
            if state[2] == 3:
                return (state[0], state[1], 2)
...
class PlanShotProblem(search.Problem):
    ...
    def h(self,node):
        ...
        "*** YOUR CODE HERE ***"
        wumpus = self.goals
        successor = self.allowed
        shots = []

        for agent in wumpus:
            for suc in successor:
                if (agent[0] == suc[0]) or (agent[1] == suc[1]):
                    shots.append(suc)
    ...
    def goal_test(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        #wumpus may be in a goal state
        wumpus = self.goals

        if state in self.goals:
            return False

        for location in wumpus:
            #if the x's match
            if location[0] == state[0]:
                #wumpus y location greater than my y and we are going north
                if ((location[1] > state[1]) and state[2] == 0):
                    return True
                #wumpus y location less than my y and going south
                if ((location[1] < state[1]) and state[2] == 2):
                    return True
            #if y's match
            if location[1] == state[1]:
                #wumpus x less than my x and going west
                if ((location[0] < state[0]) and state[2] == 1):
                    return True
                #wumpus x greater than my x and going east
